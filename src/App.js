...
// ---- fixed: no spread deps in effect array ----
...
function beep(type = "ok") {
  ...
  if (ctx.resume) try { ctx.resume(); } catch { }
...
const TopBar = React.memo(({ onAdd, onArchiveOpen, archivedCount, zoomed, onBack }) => (
  <div className="flex items-center justify-between w-full px-3 py-2 md:px-4 md:py-3 border-b border-black/5 sticky top-0 z-20 bg-background/80 backdrop-blur">
    <div className="flex items-center gap-2">
      {zoomed && (<button onClick={onBack} className="p-2 rounded-2xl hover:bg-black/5 transition"><CornerUpLeft className="w-5 h-5" /></button>)}
      <h1 className="text-lg md:text-xl font-semibold">Compass-Q</h1>
    </div>
    <div className="flex items-center gap-2 md:gap-3">
      <button onClick={onAdd} className="flex items-center gap-2 px-3 py-2 rounded-2xl bg-primary text-primary-foreground hover:opacity-90 shadow-md active:translate-y-px transition"><Plus className="w-4 h-4" /><span className="hidden sm:inline">Add</span></button>
      <button onClick={onArchiveOpen} className="flex items-center gap-2 px-3 py-2 rounded-2xl bg-muted hover:bg-muted/80 shadow-md transition"><Inbox className="w-4 h-4" /><span className="hidden sm:inline">Completed</span><span className="ml-1 text-xs px-1.5 py-0.5 rounded bg-background border">{archivedCount}</span></button>
    </div>
  </div>
));
...
const AddTaskModal = ({ open, onClose, onCreate }) => {
  ...
  useEffect(() => {
    if (open) { setTitle(""); setImportant(true); setUrgent(false); setHours(24); }
  }, [open]);

  if (!open) return null;
  return (
    <div className="fixed inset-0 z-30 grid place-items-center bg-black/40 p-3" data-modal>
      <div className="w-full max-w-md rounded-[24px] bg-gradient-to-br from-white to white/90 border border-black/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.6),0_10px_20px_rgba(0,0,0,0.15)] text-foreground modal-light">
        ...
        <div className="p-4 md:p-5 flex justify-end gap-2 border-t border-black/5">
          ...
          <button
            onClick={() => { if (!title.trim()) return; onCreate({ title: title.trim(), important, urgent, hours: Number(hours) }); }}
            className="px-3 py-2 rounded-2xl bg-primary text-primary-foreground shadow-md"
          >Create</button>
        </div>
      </div>
    </div>
...
const ArchiveModal = ({ open, onClose, items, onRestore, onDelete }) => {
  ...
  return (
    <div className="fixed inset-0 z-30 grid place-items-center bg-black/40 p-3" data-modal>
      <div className="w-full max-w-2xl rounded-[28px] bg-gradient-to-br from-white to white/90 border border-black/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.6),0_16px_32px_rgba(0,0,0,0.2)] text-foreground modal-light">
              ...
              {items.map(t => (
                <li key={t.id} className="flex items-center justify-between gap-2 border border-black/10 rounded-2xl px-3 py-2 bg-white/70 shadow-[inset_0_1px_0_rgba(255,255,255,0.6),0_4px_10px_rgba(0,0,0,0.08)]">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">Completed {DATE_FMT.format(new Date(t.completedAt))}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => onRestore(t.id)} className="px-2 py-1 rounded-xl bg-muted hover:bg-muted/80 text-xs">Restore</button>
                    <button onClick={() => onDelete(t.id)} className="px-2 py-1 rounded-xl hover:bg-destructive/10 text-destructive text-xs">Delete</button>
                  </div>
                </li>
              ))}
            </ul>
...
const EditTaskModal = ({ open, onClose, task, onSave }) => {
  ...
  useEffect(() => {
    if (open && task) {
      setTitle(task.title || "");
      setUrgent(isUrgentQuadrant(task.quadrant));
      setImportant(!!task.important);
      setHours(msToHrs(Math.max(0, task.dueAt - now())) || 24);
    }
  }, [open, task]);
  ...
  const onToggleUrgent = e => {
    ...
    setUrgent(u);
    setHours(h => {
      if (u && h > URGENT_THRESHOLD_HOURS) return 6;
      if (!u && h <= URGENT_THRESHOLD_HOURS) return 48;
      return h;
    });
  };
...
const TaskCard = React.memo(({ task, selected, provided, snapshot, setSelectedId, onEdit, nowTs }) => (
  <div
    ...
    data-task-card
    className={`plate transition transform ${snapshot.isDragging ? "scale-[1.02] shadow-2xl" : ""} ${selected ? "ring-2 ring-primary" : ""}`}
  >
    <div className="flex items-center justify-between gap-2">
      <div className="min-w-0">
        <div className="font-medium truncate">{task.title}</div>
        <div className="text-[10px] text-muted-foreground">{task.important ? "Important" : "Not important"} • {taskRemainingLabel(task, nowTs)}</div>
      </div>
    </div>
  </div>
));

const Quadrant = React.memo(function Quadrant({ id, title, items, onDoubleTap, selectedId, setSelectedId, onEdit }) {
  const tapRef = useRef({ t: 0 });
  const onTap = () => {
    const t = now();
    if (t - tapRef.current.t < 300) { onDoubleTap(id); }
    tapRef.current.t = t;
  };
  ...
  return (
    <div className="h-full w-full">
      <div className="flex items-center justify-between px-2 py-1">
        ...
        <div className="text-[10px] md:text-xs text-muted-foreground">{items.length}/10</div>
      </div>
      <div onClick={onTap} className={`rounded-[28px] qpanel ${qClass} h-[calc(100%-1.75rem)]`}>
        <Droppable droppableId={id} type="TASKS">
          {(provided, snapshot) => (
            <div ref={provided.innerRef} {...provided.droppableProps} className={`h-full max-h-full overflow-auto grid gap-2 content-start p-2 md:p-3 ${snapshot.isDraggingOver ? "ring-2 ring-primary/40" : ""}`}>
...
function taskRemainingLabel(task, nowTs) {
  const ms = Math.max(0, task.dueAt - nowTs);
  const hrs = Math.floor(ms / 3600000);
  const mins = Math.floor((ms % 3600000) / 60000);
  return `${hrs}h ${mins}m left`;
}

function groupByQuadrant(tasks) {
  const b = { [Q1]: [], [Q2]: [], [Q3]: [], [Q4]: [] };
  for (const t of tasks) { b[t.quadrant].push(t); }
  return b;
}
...
[data-modal]{color:#0b0b0e}
[data-modal] .text-muted-foreground{color:rgba(0,0,0,.6)}
[data-modal] input,[data-modal] textarea,[data-modal] select{color:#0,0,0}
[data-modal] ::placeholder{color:rgba(0,0,0,.55)}
...
export default function App() {
  ...
  useEffect(() => {
    if (!document.getElementById("compassq-styles")) {
      const s = document.createElement("style");
      s.id = "compassq-styles"; s.textContent = STYLE_CSS; document.head.appendChild(s);
    }
  }, []);

  const openAdd = useCallback(() => setAddOpen(true), []);
  const openArchive = useCallback(() => setArchiveOpen(true), []);
  const backToMatrix = useCallback(() => setZoom(null), []);
  ...
  useEffect(() => {
    ...
    const handler = e => {
      ...
      if (t.closest("[data-task-card]")) return;
      if (t.closest("[data-task-actions]")) return;
      if (t.closest("[data-modal]")) return;
      setSelectedId(null);
    };
  ...
  useEffect(() => {
    ...
    for (const t of data.tasks) {
      const d = (t.dueAt - nowTs) - URGENT_THRESHOLD_MS;
      if (d > 0) delay = Math.min(delay, d);
    }
    delay = Math.max(500, Math.min(delay, 600000));
    const timer = setTimeout(() => {
      setData(prev => {
        let changed = false;
        const tasks = prev.tasks.map(t => {
          const q = computeQuadrant(t.important, t.dueAt);
          if (q !== t.quadrant) { changed = true; return { ...t, quadrant: q }; }
          return t;
        });
  ...
  const handleOpenEditor = useCallback(task => {
    setSelectedId(task.id); setEditing(task); setEditOpen(true);
  }, []);
  ...
  function onDragEnd(result) {
    ...
    setData(prev => {
      ...
      const currentIdx = srcIndex < srcList.length && srcList[srcIndex]?.id === draggableId
        ? srcIndex
        : srcList.findIndex(t => t.id === draggableId);
  ...
  const grid = (
    <div className="grid grid-cols-1 md:grid-cols-2 grid-rows-2 gap-3 md:gap-4 p-3 md:p-4 h-[calc(100vh-56px)] md:h-[calc(100vh-64px)]">
      <Quadrant id={Q1} title={QUADRANT_META[Q1].title} items={tasksByQuadrant[Q1]} onDoubleTap={setZoom} selectedId={selectedId} setSelectedId={setSelectedId} onEdit={handleOpenEditor} />
      <Quadrant id={Q2} title={QUADRANT_META[Q2].title} items={tasksByQuadrant[Q2]} onDoubleTap={setZoom} selectedId={selectedId} setSelectedId={setSelectedId} onEdit={handleOpenEditor} />
      <Quadrant id={Q3} title={QUADRANT_META[Q3].title} items={tasksByQuadrant[Q3]} onDoubleTap={setZoom} selectedId={selectedId} setSelectedId={setSelectedId} onEdit={handleOpenEditor} />
      <Quadrant id={Q4} title={QUADRANT_META[Q4].title} items={tasksByQuadrant[Q4]} onDoubleTap={setZoom} selectedId={selectedId} setSelectedId={setSelectedId} onEdit={handleOpenEditor} />
    </div>
  );

  const zoomView = zoom ? (
    <div className="p-3 md:p-4 h-[calc(100vh-56px)] md:h-[calc(100vh-64px)]">
      <div className="h-full">
        <Quadrant id={zoom} title={QUADRANT_META[zoom].title} items={tasksByQuadrant[zoom]} onDoubleTap={() => setZoom(null)} onEdit={handleOpenEditor} selectedId={selectedId} setSelectedId={setSelectedId} />
      </div>
    </div>
  ) : null;
  ...
  useEffect(() => {
    const DEV =
      ...
      (typeof window !== "undefined" && window.location && /^(localhost|127\.0\.0\.1)/.test(window.location.hostname));
    if (!DEV) return;
    try {
      console.assert(QUADRANTS.every(q => typeof q === "string"), "Quadrant IDs must be strings");
      const soon = now() + hoursToMs(0.5);
      const later = now() + hoursToMs(48);
      ...
      const at = now() + hoursToMs(URGENT_THRESHOLD_HOURS);
      const under = now() + hoursToMs(URGENT_THRESHOLD_HOURS) - 60000;
      const over = now() + hoursToMs(URGENT_THRESHOLD_HOURS + 1);
      console.assert(computeUrgent(at) === true, "Exactly threshold should be urgent");
      console.assert(computeUrgent(under) === true, "Under threshold should be urgent");
      console.assert(computeUrgent(over) === false, "Over threshold should be not urgent");
      const ids = new Set([Q1, Q2, Q3, Q4]);
      console.assert(ids.size === 4, "droppableIds must be unique");
      console.assert(isUrgentQuadrant(Q1) && isUrgentQuadrant(Q3) && !isUrgentQuadrant(Q2) && !isUrgentQuadrant(Q4), "isUrgentQuadrant mapping");
      console.assert(isImportantQuadrant(Q1) && isImportantQuadrant(Q2) && !isImportantQuadrant(Q3) && !isImportantQuadrant(Q4), "isImportantQuadrant mapping");
      const sample = [{ id: "a", quadrant: Q1 }, { id: "b", quadrant: Q2 }, { id: "c", quadrant: Q3 }, { id: "d", quadrant: Q4 }, { id: "e", quadrant: Q1 }, { id: "f", quadrant: Q2 }];
      const gb = groupByQuadrant(sample);
      console.assert(gb[Q1].length === 2 && gb[Q2].length === 2 && gb[Q3].length === 1 && gb[Q4].length === 1, "groupByQuadrant counts");
  ...
  return (
    <>
      <div className="min-h-screen w-full bg-background text-foreground">
        <TopBar onAdd={openAdd} onArchiveOpen={openArchive} archivedCount={data.archived.length} zoomed={!!zoom} onBack={backToMatrix} />
